...
it.each([0, 48_000, 60_000])("retracts an overturned interception and restores the previous big-play clock with %ims delay", async delay => {
  const snapshot = log();
  const interception = { id: "reversed", text: "Pass INTERCEPTED", distance: 0,
  ...
  fetchLog.mockResolvedValue({ ...snapshot, playByPlay: [{ ...snapshot.playByPlay[0], plays: [
    snapshot.playByPlay[0].plays![0], { ...interception, reviewReversed: true, text: "Pass INTERCEPTED. Ruling REVERSED. Pass incomplete." },
  ] }] });
  await advance(10_000);
  expect(result.current.latestBigPlayClock).toBe("Q1 12:00");
  expect(result.current.bigPlay).toBe(false);
