...
test("fixed hotkeys and audio follow every numbered spotlight", async ({ context, page }) => {
  ...
  await context.route("https://proxy420.appspot.com/**", async route => {
    ...
    return route.fulfill({ contentType: "text/html", body: `<div class="events-list">
      ${[1, 2, 3].map(index => `<div class="event-card" data-start-ts="${Math.floor(Date.now() / 1000)}" onclick="window.location.href='/watch/${index}'">
  ...
  for (let index = 0; index < 3; index++) await page.locator(".stream-toggle").nth(index).click();
  ...
  await cards.nth(2).locator(".screen-title-bar-shell").click();
